...
class Filer {
    ...
    static addProperty(propertyName, dirName, fileName) {
        const fullPath = dirName ? path.join(__dirname, dirName, fileName) : path.join(__dirname, fileName);
    ...
    static async writeToFile(stringToWrite, filePath) {
        try {
            ...
            logger.log(`Error writing JSON data to file: ${error.message}`, true);
        }
    }

    static async updateFile(filePath, timePeriod = null, generateDataMethod = null) {
        logger.log('Filer updateFile is starting.');
        ...
        try {
            ...
            logger.log(`There was an error updating ${filePath}: ${error.message}`, true);
        }
    }
}
...
for (const [propertyName, {dirName, fileName}] of Object.entries(files)) {
    FileHandler.addProperty(propertyName, dirName, fileName);
}
